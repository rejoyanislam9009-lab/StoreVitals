...
trait Admin_Diagnostics {
	private function render_issue_view( array $scan, $allowed_areas, $title, $description ) {
		...
		$result_count_label = sprintf( _n( '%d result', '%d results', count( $results ), 'cartiloq-store-health' ), count( $results ) );
		?>
		<section class="storecheckup-section">
			...
			<form class="storecheckup-filterbar" method="get">
				...
				<label><span class="screen-reader-text"><?php echo esc_html__( 'Search diagnostics', 'cartiloq-store-health' ); ?></span><input type="search" name="storecheckup_search" value="<?php echo esc_attr( $search ); ?>" placeholder="<?php echo esc_attr__( 'Search diagnostics...', 'cartiloq-store-health' ); ?>"></label>
				<label><span class="screen-reader-text"><?php echo esc_html__( 'Filter by status', 'cartiloq-store-health' ); ?></span><select name="storecheckup_status"><option value=""><?php echo esc_html__( 'All statuses', 'cartiloq-store-health' ); ?></option><?php foreach ( array( Result::CRITICAL, Result::WARNING, Result::PASSED, Result::INFO ) as $status ) : ?><option value="<?php echo esc_attr( $status ); ?>" <?php selected( $status_filter, $status ); ?>><?php echo esc_html( ucfirst( $status ) ); ?></option><?php endforeach; ?></select></label>
				<label><span class="screen-reader-text"><?php echo esc_html__( 'Filter by area', 'cartiloq-store-health' ); ?></span><select name="storecheckup_area"><option value=""><?php echo esc_html__( 'All areas', 'cartiloq-store-health' ); ?></option><?php foreach ( $areas as $area ) : ?><option value="<?php echo esc_attr( $area ); ?>" <?php selected( $area_filter, $area ); ?>><?php echo esc_html( $this->area_label( $area ) ); ?></option><?php endforeach; ?></select></label>
				<button class="button button-primary" type="submit"><?php echo esc_html__( 'Filter', 'cartiloq-store-health' ); ?></button>
				<?php // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only navigation parameter; no state is changed. ?>
				<a class="button" href="<?php echo esc_url( $this->tab_url( isset( $_GET['view'] ) ? sanitize_key( wp_unslash( $_GET['view'] ) ) : 'issues' ) ); ?>"><?php echo esc_html__( 'Reset', 'cartiloq-store-health' ); ?></a>
			</form>
	...
	private function render_results( array $results ) {
		if ( empty( $results ) ) {
			?>
			<div class="storecheckup-empty storecheckup-empty-card"><span class="dashicons dashicons-search" aria-hidden="true"></span><h3><?php echo esc_html__( 'No matching diagnostics', 'cartiloq-store-health' ); ?></h3><p><?php echo esc_html__( 'Try clearing the filters or using a broader search.', 'cartiloq-store-health' ); ?></p></div>
			<?php
			return;
		}
		?>
