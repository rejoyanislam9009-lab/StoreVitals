...
trait Admin_Helpers {
	...
	private function area_label( $area ) {
		$labels = array(
			'products'  => __( 'Products', 'cartiloq-store-health' ),
			'inventory' => __( 'Inventory', 'cartiloq-store-health' ),
			'orders'    => __( 'Orders', 'cartiloq-store-health' ),
			'checkout'  => __( 'Checkout', 'cartiloq-store-health' ),
			'payments'  => __( 'Payments', 'cartiloq-store-health' ),
			'shipping'  => __( 'Shipping', 'cartiloq-store-health' ),
			'system'    => __( 'System', 'cartiloq-store-health' ),
			'store'     => __( 'Store', 'cartiloq-store-health' ),
		);
	...
	private function score_label( $score ) {
		if ( $score >= 90 ) {
			return __( 'Excellent', 'cartiloq-store-health' );
		}
		if ( $score >= 75 ) {
			return __( 'Good', 'cartiloq-store-health' );
		}
		if ( $score >= 50 ) {
			return __( 'Needs attention', 'cartiloq-store-health' );
		}
		return __( 'Critical', 'cartiloq-store-health' );
	}
	...
	private function headline_for_score( $score ) {
		if ( $score >= 90 ) {
			return __( 'Your store is in strong shape.', 'cartiloq-store-health' );
		}
		if ( $score >= 75 ) {
			return __( 'Your store looks healthy with a few items to review.', 'cartiloq-store-health' );
		}
		if ( $score >= 50 ) {
			return __( 'A few store health signals need attention.', 'cartiloq-store-health' );
		}
		return __( 'Important store health issues need review.', 'cartiloq-store-health' );
	}
}
