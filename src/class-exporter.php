...
final class Exporter {
	...
	public function export_csv() {
		...
		$filename = 'cartiloq-store-health-' . gmdate( 'Y-m-d-His' ) . '.csv';
		...
		if ( false === $output ) {
			wp_die( esc_html__( 'Could not open the CSV output stream.', 'cartiloq-store-health' ) );
		}

		$this->write_csv_row( $output, array( 'Cartiloq Store Health score', (int) $scan['score'] ) );
	...
	public function export_json() {
		...
		$scan     = $this->scanner->scan();
		$filename = 'cartiloq-store-health-' . gmdate( 'Y-m-d-His' ) . '.json';
		$payload  = array(
	...
	private function authorize( $nonce_action ) {
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_die( esc_html__( 'You do not have permission to export this report.', 'cartiloq-store-health' ) );
		}
